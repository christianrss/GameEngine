...
namespace GameEngine
{
    public class TerrainQuad : Object3D, IDisposable
    {
        ...
        public TerrainQuad(string sName, Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4) : base(sName)
        {
            ...
            m_Corners[0].Normal = (m_Face1Normal);
            m_Corners[1].Normal = (FaceNormals);
            m_Corners[2].Normal = (FaceNormals);
            m_Corners[3].Normal = (m_Face2Normal);
            m_Corners[4].Normal = (FaceNormals);
            m_Corners[5].Normal = (FaceNormals);
        ...
        {
            Normal.Normalize();
            m_Corners[Corner].Normal = Normal;
        }
        ...
        public class Terrain : IDisposable, ITerrainInfo
        {
                ...
                try
                {
                    for (int i = 0; i < m_xSize; i++)
                    {
                        for (int j = 0; j < m_ySize; j++)
                        {
                            ...
                            m_Quads[i, j] = new TerrainQuad(
                                sQuadName,
                                m_Elevations[i, j],
                                m_Elevations[i + 1, j],
                                m_Elevations[i, j + 1],
                                m_Elevations[i + 1, j + 1]
                            );
            ...
            public bool InlineOrSight(Vector3 Position1, Vector3 Position2)
            {
                bool los = true;
                float north;
